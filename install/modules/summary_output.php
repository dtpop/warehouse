...
if (rex::isBackend()) {
    ...
    $yf->setObjectparams('form_class', 'rex-yform summary');
    ...
    $yf->setValueField('html',['','<div class="grid content-xsmall-grid">']);
    $yf->setValueField('html',['','<div class="col-8_md-12">']);
    ...
    $yf->setValueField('submit',['send','{{ Jetzt kaufen }}','','[no_db]','','uk-button uk-margin-top']);

    $yf->setValueField('html',['','</div></div>']); // col / grid
    ...
    if ($wh_userdata['payment_type'] == 'invoice' || $wh_userdata['payment_type'] == 'prepayment') {
